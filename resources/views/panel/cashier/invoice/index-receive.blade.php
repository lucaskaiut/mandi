...
@section('content_header')
    <h1 class="title-h1">Contas a receber</h1>
    @can('fin-create')
        <div style="
                align-items: center;
                display: flex;
            ...
            @can('fin-create')
                <a href="{{route('create.invoice.receive')}}">
                    <button type="button" class="btn btn-custom">
                        Lançar Conta a Receber<i class="fa fa-plus-circle" style="padding-left:5px;"></i>
                    </button>
                </a>
            @endcan
        </div>
        ...
        <div class="main-box">
            ...
            @can('fin-list')
                <div class="col-lg-12" style="padding: 5px;">
                    <form class="form-inline" action="{{route('search.receive')}}" method="POST">
                        {{csrf_field()}}
                                ...
                                <td class="actions">
                                    @can('fin-edit')
                                        <a class="btn btn-success btn-xs"
                                           href="{{route('receivement.invoice.receive', ['id' => $invoice->id])}}"><i
                                                    class="fa fa-dollar"></i> Baixar</a>
            ...
            @can('fin-list')
                ...
                <div class="pull-right"><label>Recebido: </label>R${{number_format($totalQuitadas, 2, ',', '.')}}</div>
                <div class="pull-right"><label>Vencido: </label>R${{number_format($totalVencidas, 2, ',', '.')}} |</div>
                <div class="pull-right"><label>Total: </label>R${{number_format($total, 2, ',', '.')}} |</div>
                @if(isset($dataForm))
